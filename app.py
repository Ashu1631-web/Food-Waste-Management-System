...
if menu == "Dashboard":
    ...
    claim_value = total_claims["Total"][0]
    if pd.isna(claim_value):
        claim_value = 0

    provider_value = total_providers["Total"][0]
    if pd.isna(provider_value):
        provider_value = 0

    col1, col2, col3 = st.columns(3)

    col1.metric("🍱 Total Food Available", int(food_value))
    col2.metric("📌 Total Claims Made", int(claim_value))
    col3.metric("🏢 Total Providers", int(provider_value))
...
# ======================================================
    ...
    if food_df.empty:
        ...
        city_filter = st.sidebar.selectbox(
            "Select City",
            ["All"] + sorted(food_df["Location"].dropna().unique())
        )

        foodtype_filter = st.sidebar.selectbox(
            "Select Food Type",
            ["All"] + sorted(food_df["Food_Type"].dropna().unique())
        )

        meal_filter = st.sidebar.selectbox(
            "Select Meal Type",
            ["All"] + sorted(food_df["Meal_Type"].dropna().unique())
        )
